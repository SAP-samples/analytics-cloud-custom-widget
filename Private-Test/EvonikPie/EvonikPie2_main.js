...
*/
    async render () {
      ...
      if (!this.myDataSource || this.myDataSource.state !== 'success') {
		console.log("!! render() - myDataSource is NOT defined yet!");
        return
      }
	  

		// Added by HM now:
		console.log("-- render() - myDataSource is defined now!");
		...
		try {
			...
			for (const feedEntry of this.myDataSource.metadata.feeds.dimensions.values) {
				...
				const dimensionEntryId = this.myDataSource.metadata.dimensions[feedEntry].id;
				const dimensionEntryDescr = this.myDataSource.metadata.dimensions[feedEntry].description;
			...
			for (const feedEntry of this.myDataSource.metadata.feeds.measures.values) {
				const measureId = this.myDataSource.metadata.mainStructureMembers[feedEntry].id;
				const measureLabel = this.myDataSource.metadata.mainStructureMembers[feedEntry].label;
			...
			for (const d of this.myDataSource.data) {
				...
				for (const feedEntryDim of this.myDataSource.metadata.feeds.dimensions.values) {
				...
				for (const feedEntryMeas of this.myDataSource.metadata.feeds.measures.values) {
...
/*
	  ...
	  const dimension = this.myDataSource.metadata.feeds.dimensions.values[0]
      const measure = this.myDataSource.metadata.feeds.measures.values[0]
      const data = this.myDataSource.data.map(data => {
        return {
...
*/
	  //const dimensionZero = this.myDataSource.metadata.feeds.dimensions.values[0];
	  
	  const noMeasures = this.myDataSource.metadata.feeds.measures.values.length;
	  console.log("+-+- noMeasures: "+noMeasures+", chartTitle: "+chartTitle);
	  
	 // const chartTitle = this.myDataSource.data[dimensionZero].label;
